...
n_sectors = 10
t = np.load('data/homogeneous_ic2/t.npy')

sol_1 = np.load('data/homogeneous_ic2/0.8/homogeneous_ic2_m0.8_ic0_c.npy')
...
sol_2 = np.load('data/homogeneous_ic2/0.878/homogeneous_ic2_m0.878_ic0_c.npy')
...
sol_3 = np.load('data/homogeneous_ic2/0.956/homogeneous_ic2_m0.956_ic0_c.npy')
...
sol_4 = np.load('data/homogeneous_ic2/1.0/homogeneous_ic2_m1.0_ic0_c.npy')
...
means = np.load('results/degrees_funcnet_homogeneous_ic2_10s_no_noise'
                '/degrees_funcnet_homogeneous_ic2_10s_no_noise_means_cart_in.npy')
stds = np.load('results/degrees_funcnet_homogeneous_ic2_10s_no_noise'
               '/degrees_funcnet_homogeneous_ic2_10s_no_noise_stds_cart_in.npy')
m_vars = np.load('results/funcnet_homogeneous_ic2_10s_no_noise/m-_vars.npy')
...
node_array = np.load(
    'results/clusters_funcnet_homogeneous_ic2_10s_no_noise/ic_ic0'
    '/cluster_ic_ic0_funcnet_homogeneous_ic2_10s_no_noise.npy')
plot_components(node_array, ax3, plot_legend=True, ylabel='SCC',
              settings='overview1')
print(f'Cluster splits: node 3 at m = {m_vars[-73]:.3f}, '
      f'nodes 2 and 4 splitting off around m = {m_vars[-41]:.3f} '
      f'and reattach at m = {m_vars[-12]:.3f}')
...
path_to_nw = 'results/funcnet_homogeneous_ic2_10s_no_noise/0.8/Am-_m0.8_ic0/G_cartesian'
...
path_to_nw = 'results/funcnet_homogeneous_ic2_10s_no_noise/0.8808080808080808/Am-_m0.8808080808080808_ic2' \
             '/G_cartesian'
G_1 = nx.read_edgelist(path_to_nw, create_using=nx.DiGraph)
ssc = list(nx.strongly_connected_components(G_1))
G_c = nx.condensation(G_1, ssc)
...
path_to_nw = 'results/funcnet_homogeneous_ic2_10s_no_noise/1.0/Am-_m1.0_ic24/G_cartesian'
G_1 = nx.read_edgelist(path_to_nw, create_using=nx.DiGraph)
ssc = list(nx.strongly_connected_components(G_1))
G_c = nx.condensation(G_1, ssc)
...
coord2 = transFigure.transform(ax21.transData.transform([0.801,11.5]))
...
coord2 = transFigure.transform(ax21.transData.transform([0.878,11.5]))
arrow = patches.FancyArrowPatch(
    coord1,  # posA
    coord2,  # posB
    shrinkA=0,  # so tail is exactly on posA (default shrink is 2)
...
coord2 = transFigure.transform(ax21.transData.transform([0.956,11.5]))
arrow = patches.FancyArrowPatch(
    coord1,  # posA
    coord2,  # posB
    shrinkA=0,  # so tail is exactly on posA (default shrink is 2)
...
coord2 = transFigure.transform(ax21.transData.transform([0.999,11.5]))
arrow = patches.FancyArrowPatch(
    coord1,  # posA
    coord2,  # posB
    shrinkA=0,  # so tail is exactly on posA (default shrink is 2)
...
coord2 = transFigure.transform(ax3.transData.transform([40, -0.7]))
arrow = patches.FancyArrowPatch(
    coord1,  # posA
    coord2,  # posB
    shrinkA=0,  # so tail is exactly on posA (default shrink is 2)
...
plt.savefig('figure_3.png', dpi=resolution, bbox_inches="tight")
plt.show()
